		      ...
		      unsigned short btype, struct pmfs_blocknode **start_hint)
{
	...
	pmfs_error_mng(sb, "Unable to free block %lx\n", blocknr);
...
int __pmfs_new_block(struct super_block *sb, unsigned long *blocknr,
	unsigned short btype, int zero,int locked)
{
	struct pmfs_sb_info *sbi = PMFS_SB(sb);
	struct list_head *head = &(sbi->block_inuse_head);
...
int pmfs_new_block(struct super_block *sb, unsigned long *blocknr,
	...
	__pmfs_new_block(sb,blocknr,btype,zero,1);
}
...
unsigned long current360;

int pmfs_new_block_alt(struct super_block *sb, unsigned long *blocknr,
	unsigned short btype, int zero)
{
	return 0;
	/*
	...
	if (found == 1) {
		sbi->num_free_blocks = sbi->num_free_blocks;
	}	
