...
static void pmfs_clean_journal(struct super_block *sb, bool unmount)
{
	...
	uint32_t new_head,last_valid_head, tail;
	uint16_t gen_id;
	...
	//printk("leaving journal cleaning %d %d\n", head, tail);
}
...
inline int pmfs_add_block_to_free( pmfs_transaction_t *trans, unsigned long blk){
		...
		pmfs_transaction_t *trans, void *addr, uint64_t addr_offset, uint16_t size, u8 type)
{
	...
	for (i = 0; i < num_les; i++) {
		...
		le_start += le_size;
		le++;
		emulate_latency_ns(((sizeof(pmfs_logentry_t) + le_size)/8) * M_PCM_LATENCY_WRITE);
	}
