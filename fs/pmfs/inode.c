...
static int pmfs_new_data_block_alt(struct super_block *sb, struct pmfs_inode *pi,
		unsigned long *blocknr, int zero)
{	
	unsigned int data_bits = blk_type_to_shift[pi->i_blk_type];
	int errval = __pmfs_new_block(sb, blocknr, pi->i_blk_type, zero, 0);
		...
		__le64 root, u32 height, u32 btype, unsigned long last_blocknr)
{
	...
	//printk("freeing subtree\n");
	if (!root)
		return 0;

	//printk("freeing subtree height %d\n",height);
	if (height == 0) {
...
static int rec_cow_block(pmfs_transaction_t *trans,
	struct super_block *sb, struct pmfs_inode *pi, __le64 block, u32 height,
	unsigned long blocknr)
{
	int errval, le_size;
	unsigned int meta_bits = META_BLK_SHIFT, node_bits;
	__le64 *node;
	unsigned long new_blk, next_blk,old_blk;
	unsigned int index;
	...
	if (height == 1) {
			errval = pmfs_new_data_block_alt(sb, pi, &new_blk,0);
			//printk("XIP_COW - rec_alloc_blocks - height == 1!\n");
			if (errval) {
				pmfs_dbg_verbose("alloc data blk failed"
					" %d\n", errval);
				/* For later recovery in truncate... */
...
int pmfs_cow_block(pmfs_transaction_t *trans, struct super_block *sb,
	struct pmfs_inode *pi, unsigned long file_blocknr)
{
	...
	//printk("XIP_COW - __pmfs_cow_block - entered! height: %d\n",height);
	pmfs_dbg_verbose("alloc_blocks height %d fil_blocknr %lx num %x, "
		   "first blocknr 0x%lx,\n",
		   pi->height, file_blocknr, 1, blocknr);
...
static void pmfs_update_inode(struct inode *inode, struct pmfs_inode *pi)
{
	...
	emulate_latency_ns(44 * M_PCM_LATENCY_WRITE);
...
void pmfs_evict_inode(struct inode *inode)
{
	...
	int err = 0;
	//printk("evict_inode - inode number: %ld\n",inode->i_ino);
	//printk("root: %lx\n",pi->root);
...
retry:
	...
	inode->i_ino = ino;
	//printk("new_inode - inode number: %ld\n",ino);
...
static int pmfs_writepage(struct page *page, struct writeback_control *wbc)
{
	printk(KERN_NOTICE "XIP_COW - entered writepage function\n");
	return nobh_writepage(page, NULL, wbc);
}


static int pmfs_writepages(struct address_space *mapping, struct writeback_control *wbc)
{	
	printk(KERN_NOTICE "XIP_COW - entered writepages\n");
	return mpage_writepages(mapping, wbc, NULL);
}

const struct address_space_operations pmfs_aops_xip = {
	.writepage		= pmfs_writepage,
	.writepages		= pmfs_writepages,
