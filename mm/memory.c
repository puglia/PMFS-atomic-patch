...
again:
	...
	/*if(is_xip_cow){
		printk(KERN_NOTICE "XIP_COW - zap_pte_range - passed by the loop!\n");
		force_flush = 1;	
	}*/
		...
		struct vm_area_struct **vmas, int *nonblocking)
{
	...
	if (!nr_pages)
		return 0;
	//printk(KERN_NOTICE "XIP_COW - __get_user_pages entered \n");
	...
	do {
		...
		do {
			...
			while (!(page = follow_page_mask(vma, start,
				...
				//printk(KERN_NOTICE "XIP_COW - handle_mm_fault entered \n");
	...
	__releases(ptl)
{
	...
	if (!old_page) {
		 ...
		 */
		if ((vma->vm_flags & (VM_WRITE|VM_SHARED|VM_XIP_COW)) ==
				     (VM_WRITE|VM_SHARED))
			goto reuse;
		goto gotten;
	}
...
gotten:
	...
	if (is_zero_pfn(pte_pfn(orig_pte))) {
		...
		if(is_xip_cow){
			//printk(KERN_NOTICE "XIP_COW - do_wp_page - copying page\n");
	...
	if (likely(pte_same(*page_table, orig_pte))) {
		...
		} else if(!(vma->vm_flags & VM_XIP_COW))
			inc_mm_counter_fast(mm, MM_ANONPAGES);
		flush_cache_page(vma, address, pte_pfn(orig_pte));
		...
		pgoff_t pgoff, unsigned int flags, pte_t orig_pte)
{
	 ...
	 */
	if (is_xip_cow || ((flags & FAULT_FLAG_WRITE) && !(vma->vm_flags & VM_SHARED))) {
		//if(is_xip_cow)
		//	printk(KERN_NOTICE "XIP_COW - __do_fault - entered xip_cow branch in __do_fault\n");
		//else 
			if(!is_xip_cow)
				if (unlikely(anon_vma_prepare(vma)))
					return VM_FAULT_OOM;
		...
		//if(is_xip_cow)
		//	printk(KERN_NOTICE "XIP_COW - __do_fault - cow_page allocated");
		if (!cow_page)
			return VM_FAULT_OOM;
	...
	page = vmf.page;
	if (flags & FAULT_FLAG_WRITE) {
		if (!(vma->vm_flags & VM_SHARED) || is_xip_cow) {
			//if(is_xip_cow)
			//	printk(KERN_NOTICE "XIP_COW - __do_fault - setting up new page...");
			page = cow_page;
			anon = !is_xip_cow;
			copy_user_highpage(page, vmf.page, address, vma);
			__SetPageUptodate(page);
			//if(is_xip_cow)
			//	printk(KERN_NOTICE "XIP_COW - __do_fault - page setup completed!");
		} else {
			/*
	...
	if (dirty_page) {
		...
		//if(is_xip_cow)
		//	printk(KERN_NOTICE "XIP_COW - __do_fault - dirtying page...");
		...
		//if(is_xip_cow)
		//	printk(KERN_NOTICE "XIP_COW - __do_fault - page marked as dirty!");
		/* file_update_time outside page_lock */
...
int pte_follow(struct mm_struct *mm, unsigned long address, pte_t **ptepp, spinlock_t **ptlp){

	return follow_pte(mm,address,ptepp,ptlp);
}
